...
function isWrongMagnificCandidate(candidateUrl, pageUrl, context='') {
  ...
  const txt = String(candidateUrl || '') + ' ' + String(context || '');
  const pageIds = [...txt.matchAll(/\/free-(?:ai-image|photo)\/[^\s"'<>)]*?_(\d+)(?:\.htm|\.(?:jpg|jpeg|png|webp|avif))/gi)].map(m => m[1]);
  if (pageIds.some(id => id !== info.id)) return true;
  if (/cross[_-]?selling|related|recommend|similar|suggested/i.test(txt) && !txt.includes(info.id)) return true;
  return false;
}
...
function collectDirectUrls(text, base) {
  ...
  for (const u of raw.match(/https?:\/\/[^\s"'<>]+/gi) || []) {
    ...
    if (isHttp(cleaned) && (IMAGE_EXT.test(cleaned) || /cloudinary|cdn|image|photo|media|upload/i.test(cleaned)) && !BAD.test(cleaned) && !isWrongMagnificCandidate(cleaned, base, raw)) out.push(cleaned);
  }
  return out;
}
...
async function resolve(input) {
  ...
  const strict = filterExactPageCandidates(uniq([
    ...parseMeta(html, url),
    ...parseJsonLd(html, url),
    ...bestFromImgTags(html, url),
    ...collectDirectUrls(html, url)
  ]), url).filter(u => isHttp(u) && !BAD.test(u));
