...
function webpCloudinaryUrl(url){
  ...
  if(url.includes('/image/upload/')){
    // Full-size HD WebP: no width/height/crop transformation is added, so original dimensions stay same.
    // q_100 avoids WebP compression blur; e_sharpen gently improves soft/blurred sources without changing size.
    const clean = url.replace('/image/upload/','/image/upload/f_webp,q_100,e_sharpen:80/');
    return clean.replace(/\.(jpg|jpeg|png|gif|avif|webp)(\?.*)?$/i,'.webp');
  }
  return url;
}
...
async function uploadToCloudinary(input, title='image'){
  ...
  if(input instanceof Blob){
    // Keep the real source format name. Do not label JPG/PNG blobs as .webp before Cloudinary reads them.
