...
async function extractImageFromPageUrl(pageUrl){
  ...
  const fromHtml = collectUrlCandidatesFromText(html, pageUrl).find(u=>isLikelyDirectImageUrl(u) && !isWrongMagnificCandidate(u, pageUrl, html));
...
async function extractImageViaReader(pageUrl){
  ...
  const direct = collectUrlCandidatesFromText(txt, pageUrl).find(u=>isLikelyDirectImageUrl(u) && !isWrongMagnificCandidate(u, pageUrl, txt));
  if(direct && !isWrongMagnificCandidate(direct, pageUrl, script.textContent||'')) return absolutizeUrl(direct, pageUrl);
